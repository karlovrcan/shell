...
//Bultin command; program to shutdown 
int lsh_shutdown(char **args)
{
	char ch;
...
//Bultin command; removing empty directory
int lsh_rmdir(char **args)
{
	char dirName[16];
	char cmd[32] = { 0 };
...
// Bultin commnd; path to current file
int lsh_pwd(char **args)
{
	char cwd[1024];
...
// Bultin command; current date and time
int lsh_date(char **args)
{
	char* time_now;
	time_t t = time(NULL);
	time_now = ctime(&t);
	printf("Current time is: %s", time_now);
	return 1;
}
